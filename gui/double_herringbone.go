...
func NewDoubleHerringboneTab(configuration *configuration.Configuration) *DoubleHerringboneTab {
	t := &DoubleHerringboneTab{
		configuration: configuration,
	}
	doubleHerringbone := fltk.NewGroup(20, 30, 760, 550, "Double Herringbone")
	y := 40
	...
	doubleHerringbone.End()

	doubleHerringbone.Resizable(t.portalList)
	return t
}

func (t *DoubleHerringboneTab) OnSearchPressed() {
	...
	go func() {
		b0, b1, spine0, spine1 := lib.LargestDoubleHerringbone(t.portals, []int{}, 8, progressFunc)
		if t.mapWindow != nil {
			t.mapWindow.SetPaths([][]lib.Portal{lib.DoubleHerringbonePolyline(b0, b1, spine0, spine1)})
		}
	}()
}

func (t *DoubleHerringboneTab) OnAddPortalsPressed() {
	...
	if t.mapWindow == nil {
		t.mapWindow = NewMapWindow("Double Herringbone")
	} else {
		t.mapWindow.Show()
	}
...
func (t *DoubleHerringboneTab) PortalListDrawCallback(context fltk.TableContext, row, column, x, y, w, h int) {
	switch context {
