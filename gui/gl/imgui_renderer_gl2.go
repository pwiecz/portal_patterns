...
//go:embed gl-shader/main_gl2.vert
var vertexShader string

//go:embed gl-shader/main_gl2.frag
...
func (renderer *OpenGL2s) createDeviceObjects() {
	...
	{
		var status int32
		gl.GetShaderiv(renderer.vertHandle, gl.COMPILE_STATUS, &status)
		if status == gl.FALSE {
			var logLength int32
			gl.GetShaderiv(renderer.vertHandle, gl.INFO_LOG_LENGTH, &logLength)
			log := strings.Repeat("\x00", int(logLength+1))
			gl.GetShaderInfoLog(renderer.vertHandle, logLength, nil, gl.Str(log))

			panic(fmt.Errorf("failed to compile %v", log))
		}
	}
	gl.CompileShader(renderer.fragHandle)
	{
		var status int32
		gl.GetShaderiv(renderer.fragHandle, gl.COMPILE_STATUS, &status)
		if status == gl.FALSE {
			var logLength int32
			gl.GetShaderiv(renderer.fragHandle, gl.INFO_LOG_LENGTH, &logLength)
			log := strings.Repeat("\x00", int(logLength+1))
			gl.GetShaderInfoLog(renderer.fragHandle, logLength, nil, gl.Str(log))
	...
	var status int32
	gl.GetProgramiv(renderer.shaderHandle, gl.LINK_STATUS, &status)
	if status == gl.FALSE {
		var logLength int32
		gl.GetProgramiv(renderer.shaderHandle, gl.INFO_LOG_LENGTH, &logLength)

		log := strings.Repeat("\x00", int(logLength+1))
		gl.GetProgramInfoLog(renderer.shaderHandle, logLength, nil, gl.Str(log))

		panic(fmt.Errorf("failed to link program: %v", log))
	}

	renderer.attribLocationTex = gl.GetUniformLocation(renderer.shaderHandle, gl.Str("Texture"+"\x00"))
	if renderer.attribLocationTex < 0 {
		panic(renderer.attribLocationTex)
	}
...
func (renderer *OpenGL2s) createFontsTexture() {
	// Build texture atlas
	io := imgui.CurrentIO()
...
func (renderer *OpenGL2s) destroyFontsTexture() {
