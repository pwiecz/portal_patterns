...
func NewMainWindow(conf *configuration.Configuration) *MainWindow {
	...
	searchSaveCopyPack := fltk.NewFlex(0, 0, 700, 30)
	searchSaveCopyPack.SetType(fltk.ROW)
	searchSaveCopyPack.SetSpacing(5)
	rightPack.Fixed(searchSaveCopyPack, 30)
	...
	rightPack.Fixed(w.solutionLabel, 30)

	w.progress = fltk.NewProgress(0, 0, 700, 30)
	w.progress.SetSelectionColor(0x4444ff00)
	w.lastProgressUpdate = time.Now()
	rightPack.Fixed(w.progress, 30)

	portalListPack := fltk.NewFlex(0, 0, 700, 590)
	portalListPack.SetType(fltk.ROW)
