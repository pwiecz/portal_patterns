...
func (s thickTrianglesPureScorer) scoreTrianglePure(a, b, c portalData, level int, portals []portalData) float32 {
	...
	if level == 2 {
		...
		abDistance := q1.Distance(center.LatLng)
		q2 := newDistanceQuery(b.LatLng, c.LatLng)
		bcDistance := q2.Distance(center.LatLng)
		q3 := newDistanceQuery(a.LatLng, c.LatLng)
		acDistance := q3.Distance(center.LatLng)
		return float32(
			min(abDistance, bcDistance, acDistance) * RadiansToMeters)
	}
	return min(
