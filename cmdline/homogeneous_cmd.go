...
func (h *homogeneousCmd) Run(args []string, numWorkers int, output io.Writer, progressFunc func(int, int)) {
	...
	if *h.largestArea && *h.smallestArea || *h.largestArea && *h.mostEquilateral || *h.smallestArea && *h.mostEquilateral {
		log.Fatalln("only one of -largest_area -smallest_area -most_equilateral can be specified at the same time")
	}
